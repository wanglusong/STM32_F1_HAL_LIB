...
#define IDLE_PAGE_NUMBER           lv_ech_idle_page_num

//设置idle界面当前页
#define IDLE_PAGE_CURRENT          lv_ech_idle_page_1
...
#define LV_ech_IDLE_PAE_MAX_MSG_SIZE (20)

typedef struct{
	lv_ech_idle_page2_display_t state;
	char title[64];
	char second_line_text_1[64];
	char second_line_text_2[64];
	char third_line_text_1[64];
	char third_line_text_2[64];
} lv_ech_idle_page2_msg_content_t;

typedef struct{
	int reader;
	int writer;
	int normal_msg_count;
	lv_ech_idle_page2_msg_content_t msg[LV_ech_IDLE_PAE_MAX_MSG_SIZE];
	lv_ech_idle_page2_msg_content_t msg_normal_info;
	lv_ech_idle_page2_msg_content_t msg_warnning_info;
	lv_ech_idle_page2_msg_content_t msg_login_info;
	lv_ech_idle_page2_msg_content_t msg_audio;
	lv_ech_idle_page2_msg_content_t msg_join_group;
	lv_ech_idle_page2_msg_content_t msg_list_update;
	lv_ech_idle_page2_msg_content_t msg_speak;
	lv_ech_idle_page2_msg_content_t msg_tone;
	lv_ech_idle_page2_msg_content_t msg_tts;
	lv_ech_idle_page2_msg_content_t msg_listen;
} lv_ech_idle_page2_msg_t;

typedef enum {
	lv_ech_idle_page_1,
	lv_ech_idle_page_2,
	lv_ech_idle_page_num
} lv_ech_idle_page_e;
...
*
*************************************************/
static lv_obj_t * lv_ech_idle_create(lv_obj_t *obj);
static void lv_ech_idle_prepare_destory(lv_obj_t *obj);
static lv_res_t lv_ech_idle_signal_func(struct _lv_obj_t * obj, lv_signal_t sign, void * param);
static bool lv_ech_idle_design_func(struct _lv_obj_t * obj, const lv_area_t * mask_p, lv_design_mode_t mode);
static char * lv_ech_idle_get_date(lv_ech_time_t *time);
static char * lv_ech_idle_week_map(const int wday);
static void lv_ech_idle_time_task(void);
static char lv_ech_idle_next_page(void);
static void lv_ech_idle_page_1_show(void);
static void lv_ech_idle_page_1_hide(void);
static void lv_ech_idle_page_2_show(void);
static void lv_ech_idle_page_2_hide(void);
static void lv_ech_idle_page_2_init(void);
static void lv_ech_idle_page_task_cb(lv_task_t * task);
...
*
*************************************************/
lv_ech_activity_t *activity_idle = NULL;
static lv_task_t * idle_page_task = NULL;
static lv_ech_idle_page2_msg_t *idle_page_task_msg = NULL;
static lv_ech_idle_page2_display_t page2_display_state = lv_ech_idle_page2_normal_info;
...
{
    {lv_ech_idle_page_1_show, lv_ech_idle_page_1_hide},
    {lv_ech_idle_page_2_show, lv_ech_idle_page_2_hide},
};
...
*
*************************************************/
static lv_obj_t * lv_ech_idle_create(lv_ech_display_t *display)
{
    idle_total_page = sizeof(idle_display_funcs)/sizeof(idle_display_funcs[0]);
    idle_old_page = idle_current_page;
	
	lv_ech_status_bar_task_ext_add(lv_ech_idle_time_task);
    return NULL;
}
...
static lv_res_t lv_ech_idle_signal_func(struct _lv_obj_t * obj, lv_signal_t sign, void * param)
{
//	OSI_LOGI(0, "[ech][signal][idle] sign <- %d\n", sign);
//	OSI_LOGI(0, "[ech][signal][idle] param <- 0x%p\n", param);
	switch(sign)
	{
...
static char * lv_ech_idle_get_date(lv_ech_time_t * time)
{
	...
	sprintf(data_str,"%d月%d日 星期%s",time->tm_mon, time->tm_mday, lv_ech_idle_week_map(time->tm_wday));
	return data_str;
}
...
static void lv_ech_idle_time_task(void)
{
	...
	static bool isCreatedechTask = false;
	ech_MMI_MODEM_PLMN_RAT network_type = MMI_MODEM_PLMN_RAT_UNKNOW;
	int8_t net_type_name[64];
	memset(big_clock_str, 0 , 10);
	if(isInit == false)
	{
      isInit = true;
	  screen_h = lv_ech_get_display_height(activity_idle->display);
	  idle_clock_style = (lv_style_t *)ech_setting_conf->theme.current_theme->style_idle_big_clock;
      idle_big_clock = lv_label_create(activity_idle->display, NULL);
      lv_obj_add_style(idle_big_clock, LV_OBJ_PART_MAIN, idle_clock_style);
      idle_date_label = lv_label_create(activity_idle->display,NULL);
      style = (lv_style_t *)&theme_white_style_idle_date_label;
    ...
    {
        lv_obj_set_hidden(lv_ech_status_bar, true);
    }
    else
    {
        lv_obj_set_hidden(lv_ech_status_bar, false);
    }

	if(isCreatedechTask == false)
	{
		if(network_type == MMI_MODEM_PLMN_RAT_LTE || network_type == MMI_MODEM_PLMN_RAT_GSM || network_type == MMI_MODEM_PLMN_RAT_UMTS)
		{
			isCreatedechTask = true;
		}
	}
...
static void lv_ech_idle_page_2_show(void)
{
    ...
    if(page2_display_state == lv_ech_idle_page2_login_info)
    {
	    obj_content[obj_write] = idle_user_label;
	    obj_content_text[obj_write++] = idle_user_label_text;
    }
    else if(page2_display_state == lv_ech_idle_page2_audio)
    {
	    obj_content[obj_write] = idle_user_label;
	    obj_content_text[obj_write++] = idle_user_label_text;
    ...
    else if(page2_display_state == lv_ech_idle_page2_join_group)
    {
	    obj_content[obj_write] = idle_user_label;
	    obj_content_text[obj_write++] = idle_user_label_text;
	    obj_content[obj_write] = idle_group_label;
    ...
    else if(page2_display_state == lv_ech_idle_page2_list_update)
    {
	    obj_content[obj_write] = idle_user_label;
	    obj_content_text[obj_write++] = idle_user_label_text;
    }
    else if(page2_display_state == lv_ech_idle_page2_speak)
    {
	    obj_content[obj_write] = idle_user_label;
	    obj_content_text[obj_write++] = idle_user_label_text;
	    obj_content[obj_write] = idle_group_label;
    ...
    else if(page2_display_state == lv_ech_idle_page2_tone)
    {
	    obj_content[obj_write] = idle_user_label;
	    obj_content_text[obj_write++] = idle_user_label_text;
    }
    else if(page2_display_state == lv_ech_idle_page2_tts)
    {
	    obj_content[obj_write] = idle_user_label;
	    obj_content_text[obj_write++] = idle_user_label_text;
    }
    else if(page2_display_state == lv_ech_idle_page2_listen)
    {
	    obj_content[obj_write] = idle_user_label;
	    obj_content_text[obj_write++] = idle_user_label_text;
	    obj_content[obj_write] = idle_group_label;
    ...
    else if(page2_display_state == lv_ech_idle_page2_normal_info)
    {
    ...
    else if(page2_display_state == lv_ech_idle_page2_warnning_info)
    {
	    obj_content[obj_write] = idle_user_label;
	    obj_content_text[obj_write++] = idle_user_label_text;
    }
	...
	if(obj_write == 1)
	{
		//lv_obj_set_width(obj_content[0], lv_ech_get_display_width(activity_idle->display)/40*35);
		lv_obj_align(obj_content[0],
			activity_idle->display,
			LV_ALIGN_IN_TOP_LEFT,
			lv_ech_get_display_width(activity_idle->display)/20,
			lv_ech_get_display_height(activity_idle->display)/12*5);
	}
	else if(obj_write == 2)
	{
		lv_obj_set_width(obj_content[0], lv_ech_get_display_width(activity_idle->display)/40*35);
		lv_obj_align(obj_content[0],
			...
			LV_ALIGN_IN_TOP_LEFT,
			lv_ech_get_display_width(activity_idle->display)/20,
			lv_ech_get_display_height(activity_idle->display)/40*9);

		lv_obj_set_width(obj_content[1], lv_ech_get_display_width(activity_idle->display)/40*35);
	...
	else if(obj_write == 3)
	{
		...
		lv_obj_align(obj_content[0],
			...
			LV_ALIGN_IN_TOP_LEFT,
			lv_ech_get_display_width(activity_idle->display)/20,
			lv_ech_get_display_height(activity_idle->display)/5);
		...
		lv_obj_set_width(obj_content[2], lv_ech_get_display_width(activity_idle->display)/40*35);
	...
	{
		lv_obj_set_width(obj_content[0], lv_ech_get_display_width(activity_idle->display)/40*13);
		lv_obj_align(obj_content[0],
			activity_idle->display,
			LV_ALIGN_IN_TOP_LEFT,
			lv_ech_get_display_width(activity_idle->display)/20,
		...
		lv_obj_set_width(obj_content[1], lv_ech_get_display_width(activity_idle->display)/40*13);
		lv_obj_align(obj_content[0],
			obj_content[0],
			LV_ALIGN_OUT_BOTTOM_LEFT,
			0,
		...
		lv_obj_set_width(obj_content[2], lv_ech_get_display_width(activity_idle->display)/40*35 - lv_ech_get_display_width(obj_content[0]));
		lv_obj_align(obj_content[2],
			obj_content[0],
		...
		lv_obj_set_width(obj_content[3], lv_ech_get_display_width(activity_idle->display)/40*35 - lv_ech_get_display_width(obj_content[1]));
    ...
    {
		lv_obj_set_width(obj_content[0], lv_ech_get_display_width(activity_idle->display)/10*9);
		lv_obj_align(obj_content[0],
			activity_idle->display,
			LV_ALIGN_IN_TOP_LEFT,
			lv_ech_get_display_width(activity_idle->display)/20,
			lv_ech_get_display_height(activity_idle->display)/10);
		...
		lv_obj_set_width(obj_content[2], lv_ech_get_display_width(activity_idle->display)/40*13);
		lv_obj_align(obj_content[2],
			obj_content[1],
			LV_ALIGN_OUT_BOTTOM_LEFT,
			0,
		...
		lv_obj_set_width(obj_content[4], lv_ech_get_display_width(activity_idle->display)/40*35 - lv_ech_get_display_width(obj_content[2]));
...
static void lv_ech_idle_page_2_init(void)
{
    ...
    {
        isInit_page_2 = true;
        page2_display_state = lv_ech_idle_page2_login_info;
        style = (lv_style_t *)(ech_setting_conf->theme.current_theme->style_idle_msg_label);
        ...
        if(activity_idle->has_control == true)
        {
            lv_obj_align(idle_title_label, activity_idle->display, LV_ALIGN_IN_TOP_LEFT,
                lv_ech_get_display_width(activity_idle->display)/20,
                lv_ech_get_display_height(activity_idle->display)/15);
        }
        else
        {
            lv_obj_align(idle_title_label, activity_idle->display, LV_ALIGN_IN_TOP_LEFT,
...
static void lv_ech_idle_page_task_cb(lv_task_t * task)
{
    static lv_ech_idle_page2_msg_content_t *dest_msg = NULL;
    static bool have_warning = false;

	do{
  		if(task == NULL || task->user_data == NULL) break;

		lv_ech_idle_page2_msg_t *page2_msg = (lv_ech_idle_page2_msg_t *)task->user_data;
		lv_ech_idle_page2_msg_content_t *msg_content = &page2_msg->msg[page2_msg->reader];
		bool is_normal = false;

		if(msg_content->state <= lv_ech_idle_page2_none_msg || msg_content->state > lv_ech_idle_page2_listen)
		{
			if(page2_display_state != lv_ech_idle_page2_speak
				&& page2_display_state != lv_ech_idle_page2_listen
				&& page2_display_state != lv_ech_idle_page2_normal_info
				&& (have_warning == false || (have_warning == true && page2_display_state != lv_ech_idle_page2_warnning_info)))
			{
				...
				if(page2_msg->normal_msg_count > 21)
				{
					if(have_warning == true)
					{
						...
						lv_ech_idle_set_page2(lv_ech_idle_page2_warnning_info, content, 1);
					}
					else
					{
						dest_msg = &page2_msg->msg_normal_info;
						char *content[] = {dest_msg->second_line_text_2, dest_msg->third_line_text_2};
						lv_ech_idle_set_page2(lv_ech_idle_page2_normal_info, content, 2);
					}
		...
		page2_msg->normal_msg_count = 0;
		page2_msg->reader = (page2_msg->reader + 1) % LV_ech_IDLE_PAE_MAX_MSG_SIZE;


		if(msg_content->state == lv_ech_idle_page2_normal_info)
		{
			dest_msg = &page2_msg->msg_normal_info;
			memset(dest_msg, 0, sizeof(lv_ech_idle_page2_msg_content_t));
			dest_msg->state = lv_ech_idle_page2_normal_info;
		...
		else if(msg_content->state == lv_ech_idle_page2_warnning_info)
		{
			switch((int)page2_display_state)
			{
				case lv_ech_idle_page2_none_msg:
				case lv_ech_idle_page2_normal_info:
				case lv_ech_idle_page2_warnning_info:
				case lv_ech_idle_page2_login_info:
				case lv_ech_idle_page2_audio:
				case lv_ech_idle_page2_join_group:
				case lv_ech_idle_page2_list_update:
				case lv_ech_idle_page2_tone:
				case lv_ech_idle_page2_tts:
				{
		...
		else if(msg_content->state == lv_ech_idle_page2_login_info)
		{
			switch((int)page2_display_state)
			{
				case lv_ech_idle_page2_none_msg:
		...
		else if(msg_content->state == lv_ech_idle_page2_audio)
		{
			switch((int)page2_display_state)
			{
				case lv_ech_idle_page2_none_msg:
		...
		else if(msg_content->state == lv_ech_idle_page2_join_group)
		{
			switch((int)page2_display_state)
			{
				case lv_ech_idle_page2_none_msg:
		...
		else if(msg_content->state == lv_ech_idle_page2_list_update)
		{
			switch((int)page2_display_state)
			{
				case lv_ech_idle_page2_none_msg:
		...
		else if(msg_content->state == lv_ech_idle_page2_speak)
		{
			switch((int)page2_display_state)
			{
				case lv_ech_idle_page2_none_msg:
				...
				case lv_ech_idle_page2_tone:
				case lv_ech_idle_page2_tts:
				case lv_ech_idle_page2_speak:
				case lv_ech_idle_page2_listen:
				{
					if(msg_content->second_line_text_1[0] == 0
		...
		else if(msg_content->state == lv_ech_idle_page2_tone)
		{
			switch((int)page2_display_state)
			{
				case lv_ech_idle_page2_none_msg:
		...
		else if(msg_content->state == lv_ech_idle_page2_tts)
		{
			switch((int)page2_display_state)
			{
				case lv_ech_idle_page2_none_msg:
		...
		else if(msg_content->state == lv_ech_idle_page2_listen)
		{
			switch((int)page2_display_state)
			{
				case lv_ech_idle_page2_none_msg:
		...
		memset(msg_content, 0, sizeof(lv_ech_idle_page2_msg_content_t));
	} while(0);
...
{
	if(content == NULL || idle_page_task_msg == NULL || idle_page_task == NULL || csize < 1 || msg_type <= lv_ech_idle_page2_none_msg || msg_type > lv_ech_idle_page2_listen)
	{
		return;
	}
	...
	lv_ech_idle_page2_msg_t *page2_msg = idle_page_task_msg;
	lv_ech_idle_page2_msg_content_t *msg_content = &page2_msg->msg[page2_msg->writer];
	page2_msg->writer = (page2_msg->writer + 1) % LV_ech_IDLE_PAE_MAX_MSG_SIZE;

	memset(msg_content, 0, sizeof(lv_ech_idle_page2_msg_content_t));

    if(msg_type == lv_ech_idle_page2_login_info && csize > 0)
    {
	    text_content[text_content_write++] = msg_content->second_line_text_1;
    }
    else if(msg_type == lv_ech_idle_page2_audio && csize > 1)
    {
	    text_content[text_content_write++] = msg_content->second_line_text_1;
	    text_content[text_content_write++] = msg_content->third_line_text_1;
    }
    else if(msg_type == lv_ech_idle_page2_join_group && csize > 1)
    {
	    text_content[text_content_write++] = msg_content->second_line_text_1;
	    text_content[text_content_write++] = msg_content->third_line_text_1;
    }
    else if(msg_type == lv_ech_idle_page2_list_update && csize > 0)
    {
	    text_content[text_content_write++] = msg_content->second_line_text_1;
    }
    else if(msg_type == lv_ech_idle_page2_speak && csize > 1)
    {
	    text_content[text_content_write++] = msg_content->second_line_text_1;
	    text_content[text_content_write++] = msg_content->third_line_text_1;
    }
    else if(msg_type == lv_ech_idle_page2_tone && csize > 0)
    {
	    text_content[text_content_write++] = msg_content->second_line_text_1;
    }
    else if(msg_type == lv_ech_idle_page2_tts && csize > 0)
    {
	    text_content[text_content_write++] = msg_content->second_line_text_1;
    }
    else if(msg_type == lv_ech_idle_page2_listen && csize > 1)
    {
	    text_content[text_content_write++] = msg_content->second_line_text_1;
	    text_content[text_content_write++] = msg_content->third_line_text_1;
    }
    else if(msg_type == lv_ech_idle_page2_normal_info && csize > 1)
    {
	    if(csize > 2)
	    {
    ...
    else if(msg_type == lv_ech_idle_page2_warnning_info && csize > 0)
    {
	    text_content[text_content_write++] = msg_content->second_line_text_1;
    }
...
static void  lv_ech_idle_control_left_label_event_cb(lv_obj_t * obj, lv_event_t event)
{
	if(LV_EVENT_CLICKED == event)
	{
		//lv_ech_main_menu_open();
	}
}

static void  lv_ech_idle_control_right_label_event_cb(lv_obj_t * obj, lv_event_t event)
{
	if(LV_EVENT_CLICKED == event)
	{
		lv_ech_idle_next_page();
	}
}

static void lv_ech_idle_start_task_cb(lv_task_t *task)
{
	lv_ech_idle_time_task();
	lv_ech_idle_page_2_hide();

	idle_page_task_msg = (lv_ech_idle_page2_msg_t *)lv_mem_alloc(sizeof(lv_ech_idle_page2_msg_t));
	if(idle_page_task_msg != NULL)
	{
		memset(idle_page_task_msg, 0, sizeof(lv_ech_idle_page2_msg_t));
		idle_page_task = lv_task_create(lv_ech_idle_page_task_cb, 100, LV_TASK_PRIO_LOWEST, idle_page_task_msg);
	}
}

lv_ech_activity_t * lv_ech_create_idle(void)
{
	static lv_ech_activity_ext_t	activity_idle_ext = {ACT_ID_ECH_IDLE,
															lv_ech_idle_create,
															lv_ech_idle_prepare_destory};
	lv_ech_control_text_t control = {"菜单", "", "下一页"}; 
	if(activity_idle != NULL)
	{
		return NULL;
	}
	...
	activity_idle = lv_ech_create_activity(&activity_idle_ext,true,true,&control);
	lv_ech_activity_set_signal_cb(activity_idle, lv_ech_idle_signal_func);
	lv_ech_activity_set_design_cb(activity_idle, lv_ech_idle_design_func);
	lv_obj_set_click(activity_idle->control->left_button, true);
	lv_obj_set_event_cb(activity_idle->control->left_button, lv_ech_idle_control_left_label_event_cb);
	lv_obj_set_click(activity_idle->control->right_button, true);
	lv_obj_set_event_cb(activity_idle->control->right_button, lv_ech_idle_control_right_label_event_cb);
	
	UART1_apTrace("FUNC: %s LINE: %d [song]:idle create\r\n", __func__, __LINE__);

	lv_task_t * prv_ech_idle_start_task = lv_task_create(lv_ech_idle_start_task_cb, 300, LV_TASK_PRIO_LOWEST, NULL);
	if(prv_ech_idle_start_task != NULL)
	{
		lv_task_once(prv_ech_idle_start_task);
	}
